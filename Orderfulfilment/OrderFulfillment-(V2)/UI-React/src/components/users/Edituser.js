...
const{product_ID,product_Name,product_Price,product_Qnty,stock} = user;
   ...
   const onInputChange = e =>{
    ...
    const onSubmit = async e => {
        e.preventDefault();
        const result=await axios.post(`http://localhost:8080/api/editProductDetailsByID`,user);
    ...
    const loadUser = async () =>{
        ...
        const respData= await axios.get(`http://localhost:8080/api/getProductDetailsById/${product_ID}`);
        
        setuser(respData.data[0]);
        console.log('response id......',respData.data);
