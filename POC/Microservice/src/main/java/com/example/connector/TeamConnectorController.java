...
public class TeamConnectorController {
	...
	public Map<String, Object> startFlow(@RequestBody MessageDto MessageDto) {

		ProcessInstanceResult workflowInstanceResult = ( zeebeClient
				.newCreateInstanceCommand().bpmnProcessId("ConnectorConnector").latestVersion()
				.variables(MessageDto)).withResult().send().join();
		
		Map<String, Object> variableMap = workflowInstanceResult.getVariablesAsMap();

//		logger.info("overall variables : " + JSONObject.toJSONString(variableMap));
